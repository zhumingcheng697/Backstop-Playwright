...
module.exports = async function (page, scenario) {
  const intercept = async (route, request) => {
    const requestUrl = request.url();

    const cookiesList = await page.context().cookies([requestUrl]);
  ...
  page.route(scenario.url, (route, request) => {
    intercept(route, request);
  });
};
